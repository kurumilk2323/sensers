...
enInpactflg en_inpactFlg = NO_INPACT;
int   loop_cnt = 0;
unsigned int tskLo_cnt;
enInitflg en_Initflg = INIT_START;
...
void loop(){
  ...
  if (en_Initflg != INIT_START){
    if ((xGyro > 10) || (xGyro > 10) || (xGyro > 10)){
      Serial.println("The inpact Occer!!.");
      en_inpactFlg = INPACT;
    }
    if(en_inpactFlg == INPACT){
      loop_cnt++;
      BlinkLED(loop_cnt);
      if(loop_cnt > 20){
        en_inpactFlg = NO_INPACT;
        en_Initflg = INIT_END;
        i_count = 0;
      }
    }else{
...
// 基板LEDチカチカ
void BlinkLED(int call_cnt)
{
    // 基板LEDチカチカ
    call_cnt++;
    if(call_cnt % 2 == 0){
      digitalWrite(PIN_SHOCK_LED, LOW);
    }else{
      digitalWrite(PIN_SHOCK_LED, HIGH);
    }
}
void print_result(){
  int i,j; 
